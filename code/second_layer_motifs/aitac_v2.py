...
class motifCNN(nn.Module):
            ...
            def forward(self, input):
                ...
                predictions = torch.zeros(batch_size, 300,  141)
...
def test_model(test_loader, model, device):
    ...
    activations2 = torch.zeros(0,200,166) ########################################### poner 166
...
def get_motifs(data_loader, model, device):
    activations = torch.zeros(0, 300, 498)
    predictions = torch.zeros(0, 300, 141)
    with torch.no_grad():
        model.eval()
