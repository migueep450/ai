...
class ConvNet(nn.Module):
    def __init__(self, num_classes, num_filters):
        ...
        self.layer3 = nn.Sequential(
            ...
            nn.MaxPool2d(kernel_size=(1, 4), stride=(1,4), padding=(0,1)),
            nn.BatchNorm2d(200))
######################################################################################## If we change the measure of the sequence, we have to change the in_features value
...
class motifCNN(nn.Module):
            ...
            def forward(self, input, num_filters):
                ...
                predictions = torch.zeros(batch_size, num_filters,  141)
                ...
                for i in range(num_filters):
                    ...
                    filter_input[:,i,:,:] = filter_input.new_full((batch_size, 1, 176), fill_value=filter_means_batch[i]) ############################## no entiendo el porque, pero es algo del tamaño de os datos de entrada
...
def get_motifs(data_loader, model, device):
    num_filters=model.layer1_conv[0].out_channels
    activations = torch.zeros(0, num_filters, 498)
    predictions = torch.zeros(0, num_filters, 141)
    with torch.no_grad():
        model.eval()
